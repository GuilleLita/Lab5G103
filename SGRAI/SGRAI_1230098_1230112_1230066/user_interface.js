...
export default class UserInterface extends GUI {
    constructor(thumbRaiser) {
        ...
        */
        ...
        const selectBuildingFolder = this.addFolder("Select building");
        selectBuildingFolder.domElement.style.fontSize = fontSize;
        const selectBuildingParameters = { name: "a1" };
        const selectBuildingOptions = ["A1", "A2"];
        selectBuildingFolder.add(selectBuildingParameters, "name").options(selectBuildingOptions).onChange(name => thumbRaiser.selectBuilding(name));
        selectBuildingFolder.close();
